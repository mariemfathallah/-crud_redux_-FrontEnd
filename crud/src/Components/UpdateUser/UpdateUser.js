...
const validForm = () =>{
   ...
   return status;
    ...
    const handleSubmit = async(e)=>{
        ...
        await dispatch(UpdatedUser(id,data))
        navigate('/users')
    }
    else{
  ...
  return (
    <>
    <h1 style={{ margin: '30px', position: 'relative', top: '100px' }} >UPDATE USER</h1>
  ...
  {error.role !== '' ? <div style={{color:'red'}} className="position-relative top-100 start-0">{error.role} </div> : ''} 
      ...
      <Button variant="primary" type="submit" style={{ padding: '0.2em 5em', height: '50px', 
